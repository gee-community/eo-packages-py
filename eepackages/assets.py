...
def cloudMaskAlgorithms_Landsat(image):
    ...
    return imageWithCloud.addBands(
        ee.Image(imageWithCloud.select("cloud").divide(100)), None, True
    )
...
def getImages(g, options):
    ...
    if options and "clipBufferSizeL7" in options:
        ...
        def f(i):
            mask = (
                i.select(["green", "red", "nir", "swir"])
                .mask()
                .reduce(ee.Reducer.allNonZero())
            )
            mask = mask.focal_min(
                options["clipBufferSizeL7"], "square", "meters"
            ).reproject(i.select("nir").projection())
    ...
    def clipNegativeFootprint(i):
        return i.clip(
            i.select(0).geometry().buffer(ee.Number(clipBufferSize).multiply(-1), 1000)
        )
    ...
    def f2(i):
        return (
            i.addBands(i.multiply(0.0001).float(), i.bandNames().remove("cloud"), True)
            .copyProperties(i)
            .set(
                {
                    "SUN_ELEVATION": ee.Number(90).subtract(
                        i.get("MEAN_SOLAR_ZENITH_ANGLE")
                    )
                }
            )
            .set({"MISSION": "S2"})
        ...
        if "filterMaskedFraction" in options and options["filterMaskedFraction"]:
            ...
            def f(i):
                maskedArea = (
                    ...
                    .reduceRegion(ee.Reducer.sum(), g, ee.Number(scale).multiply(10))
                    .values()
                    .get(0)
                )
                return i.set({"maskedFraction": ee.Number(maskedArea).divide(area)})

            images = images.map(f).filter(
                ee.Filter.gt("maskedFraction", options["filterMaskedFraction"])
            )
        else:
                ...
                lambda i: i.set(
                    {
                        ...
                        .mask()
                        .reduceRegion(
                            ee.Reducer.allNonZero(), g, ee.Number(scale).multiply(10)
                        )
                        .values()
                        .get(0)
                    }
...
def mosaicByTime(images):
    ...
    def merge_matches(i):
        ...
        mosaic = (
            matchedImages.sort("system:index")
            .mosaic()
            .set({"system:footprint": matchedImages.geometry()})
        )
...
def addQualityScore(images, g, options):
    ...
    def add_quality_score(i):
        ...
        score = (
            score.reduceRegion(ee.Reducer.percentile([scorePercentile]), g, scale)
            .values()
            .get(0)
        )
...
def getMostlyCleanImages(images, g, options=None):
    ...
    images = addQualityScore(images, g, options).filter(
    ...
    if options and options.get("quality_score_cloud_threshold"):
        images = images.filter(
            ee.Filter.lte("quality_score", options["quality_score_cloud_threshold"])
        )
    # Else calculate based on available images
    else:
        images = images.sort("quality_score").limit(
            images.size().multiply(ee.Number(1).subtract(cloud_frequency)).toInt()
        )
...
def addCdfQualityScore(
    ...
    pBad = (
        images.select("q")
        .reduce(ee.Reducer.percentile([opt_thresholdMin, opt_thresholdMax]))
        .rename(["qmin", "qmax"])
    )
